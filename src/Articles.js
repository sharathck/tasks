...
const fireBaseTTSCollection = process.env.REACT_APP_FIREBASE_ARTICLES_COLLECTION;
    ...
    return (
        <div>
            <div className="app" style={{ marginBottom: '120px', fontSize: '24px' }}>
                {readerMode ? (
                    <div>
                        <button className="articles-button" onClick={handleBack}><FaArrowLeft /></button>
                        <p>{articles}</p>
                    </div>
                ) : (
                    <div>
                        {isTriggeredFromMainApp && <button className={showMainApp ? 'articles-button_selected' : 'articles-button'} onClick={() => setShowMainApp(!showMainApp)}>
                            <FaArrowLeft />
                        </button>
                        }
                        &nbsp;
                        <input
                            ...
                            max="99"
                        />
                        {isTriggeredFromMainApp && <button className={showAudioApp ? 'button_selected' : 'articles-button'} onClick={() => setShowAudioApp(!showAudioApp)}>
                            <FaPlay />
                        </button>
                        }
                        &nbsp;
                        <button className={showCompleted ? 'articles-button_selected' : 'articles-button'} onClick={() => setShowCompleted(!showCompleted)}>
                            <FaCheckDouble />
                        </button>
                        <button className='articles-button' onClick={generateDocx}><FaFileWord /></button>
                        <button className='articles-button' onClick={generateText}><FaFileAlt /></button>
                        <button className={isGeneratingTTS ? 'button_selected' : 'articles-button'} onClick={generateTTS}><FaHeadphones /></button>
                        {!isiPhone && <button className={showLiveTTS ? 'button_selected' : 'articles-button'} onClick={synthesizeSpeech}><FaVolumeUp /></button>}
                        <button className='articles-button' onClick={handleReaderMode}><FaReadme /></button>
                        <button className="articles-button" onClick={handleSignOut}>
                            <FaSignOutAlt />
                        </button>
                        <br />
                        <br />
                            ...
                            <div>
                                ...
                                <form onSubmit={handleAddTask}>
                                    <input
                                        ...
                                        autoFocus
                                    />
                                    <button className="articles-addbutton" type="submit">
                                        <FaPlus />
                                    </button>
                                </form>
                                <ul>
                                        ...
                                        .map((task) => (
                                            <li key={task.id}>
                                                {editTask === task.id ? (
                                                    ...
                                                    <>
                                                        <button className='articles-markcompletebutton' onClick={() => handleToggleStatus(task.id, task.status)}>
                                                            <FaCheck />
                                                        </button>
                                                        <span>{task.task}</span>
                                                    </>
                                ...
                                <button className="articles-button" onClick={fetchMoreArticles}>Show More</button>
                            </div>
                        )}
                        {showCompleted && (
                            <div>
                                    ...
                                    {completedTasks
                                        ...
                                        .map((task) => (
                                            <li key={task.id} className="completed">
                                                ...
                                                <button onClick={() => handleDeleteTask(task.id, task.task)} className='articles-button_delete_selected'>
                                                    <FaTrash />
                                                </button>
                                            </li>
                                        ))}
